...
if __name__ == "__main__":
    ...
    # create obj_df for evaluation (TODO: add to dataset class?!)
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If set to True, this will use the object dwell time correlation coefficient in addition to dur&amp statistics.",
    )
    parser.add_argument(
    ...
    fitnessObjR2 = args.fitnessObjR2
    job_id = args.jobID
    if fitnessFovCat:
        namestr = f"{NAME}{MODEL}_{VIDEOS}_{FEATURESET}_wFovCat_{N_init}-{N_pop}-{N_gen}_{timestamp}_{job_id}"
    elif fitnessObjR2:
        namestr = f"{NAME}{MODEL}_{VIDEOS}_{FEATURESET}_wFitnessObjR2_{N_init}-{N_pop}-{N_gen}_{timestamp}_{job_id}"
    else:
    ...
    if VIDEOS == "all":
        ...
        df_gt_obj = df_gt_obj[df_gt_obj["video"].isin(VidCom.trainset)].reset_index(drop=True)
        ...
        df_gt_obj = df_gt_obj[df_gt_obj["video"] == VIDEOS].reset_index(drop=True)
    else:
        raise Exception(
            f"videos flag should be all, train, or sgl_vid, but is {VIDEOS}!"
        )
    # only saccades larger than 0.5 dva
    ...
    def optimize_me(traj):
        ...
        if fitnessFovCat:
            ...
            model.evaluate_all_to_df()
            df_obj = model.evaluate_all_obj()
            merged_df = pd.merge(df_gt_obj, df_obj, how="outer", on=['video', 'object'], suffixes=('_GT', '_SIM'))
            merged_df.replace(np.nan, 0, inplace=True)
            mean_ground_truth = np.mean(merged_df["tot_t_GT"])
            sst = np.sum((merged_df["tot_t_GT"] - mean_ground_truth) ** 2)
            ssr = np.sum((merged_df["tot_t_GT"] - merged_df["tot_t_SIM"]) ** 2)
            # r_squared = 1 - (ssr / sst)  # 1 is best, but for us 0 is target!
            obj_cor = ssr / sst
            fitness_tuple = (ks_dur, ks_amp, obj_cor)
            ...
            print(
                ...
                f"results in ks_dur:{ks_dur}, ks_amp:{ks_amp}, obj_cor:{obj_cor}"
            )
        else:
